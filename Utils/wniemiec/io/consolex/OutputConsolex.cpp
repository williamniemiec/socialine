...
{
    const std::string finalString = add_margin(line);
...
{
    return build_margin(marginLeft) + line;
}
...
{
    const std::string finalString = add_margin(content);
...
void OutputConsolex::write_file_lines_with_enumeration(std::string file)
{
    ...
    while (std::getline(infile, line))
    {
        write_line_with_enumeration(line, lineNumber);
        lineNumber++;
    }
}

void OutputConsolex::write_line_with_enumeration(std::string line, int lineNumber) {
    std::string format = "%-" + std::to_string(MARGIN_LEFT) + "d\t%s\n";

    printf(format.c_str(), std::to_string(lineNumber).c_str(), line.c_str());
